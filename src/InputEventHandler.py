...
class ConfigurationEventPrinter(pylon.ConfigurationEventHandler):
    ...
    def OnCameraEvent(self, camera, eventHandler):
        event_id = eventHandler.GetEventID()
        timestamp = eventHandler.GetTimestamp()
        print(f"Camera event {event_id} occurred at {timestamp}")
